...
def loss_function(alpha, X, Y, theta):
    return(0.5*np.sum((Y - X @ theta)**2) + alpha/2 * np.sum((np.concatenate(([0], theta[1:])))**2))

class RidgeRegr:
    
    def __init__(self, alpha=0, learning_rate=0.1, tolerance=1e-8, max=1e5):
    ...
    def fit(self, X, Y):
        ...
        while iteracje < self.max:
            ...
            if loss_function(self.alpha, X, Y, self.theta) > loss_function(self.alpha, X, Y, theta):
