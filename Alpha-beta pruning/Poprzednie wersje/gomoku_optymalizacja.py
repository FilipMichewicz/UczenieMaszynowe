...
class Gomoku:
        ...
        for i, j in occupied:
            for r in range(i-2, i+3):
                for c in range(j-2, j+3):
                    if self.cell(r, c) == -9:
                        positions[(r, c)] = positions.get((r, c), 0) + 1
    ...
    def evaluate_position(self, row, col):
        ...
        # Row
        lines.append("".join([char[board[row, c]] for c in range(cols)]))
        
        # Columns
        lines.append("".join([char[board[r, col]] for r in range(rows)]))

        # Diagonal (left-top to right-bottom)
        s1 = min(row, col)
        lines.append(self.diagonal_as_string(board, rows, cols, row - s1, col - s1, 1, 1, char))
        
        # Diagonal (right-top to left-bottom)
        s2 = min(row, (cols - 1) - col)
        lines.append(self.diagonal_as_string(board, rows, cols, row - s2, col + s2, 1, -1, char))
    ...
    def minimax(self, depth, position, alpha, beta, maximizingPlayer, current_score):
        if maximizingPlayer:
        ...
        if maximizingPlayer:
            ...
            for position in best_positions:
                row, col = position
                score_before = self.evaluate_position(row, col)
                self.board[row, col] = self.player
                score_after = self.evaluate_position(row, col)
                
                new_score = current_score - score_before + score_after
                eval = self.minimax(depth - 1, position, alpha, beta, False, new_score)
            ...
            for position in best_positions:
                ...
                new_score = current_score - score_before + score_after
                eval = self.minimax(depth - 1, position, alpha, beta, True, new_score)
    ...
    def ai_move(self):
        ...
        evaluated_score = self.evaluate_board()
        ...
        for position in best_positions:
            ...
            current_evaluation = evaluated_score - score_before + score_after
            score = self.minimax(self.depth-1, position, alpha, np.inf, False, current_evaluation)
